...
type ProductType = (typeof products)[number] | (typeof productsshirts)[number];
...
export default function FavoritesPage() {
  ...
  const favoriteProducts: ProductType[] = [...favoriteSuits, ...favoriteShirts];
      ...
      <div className="container mx-auto p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {uniqueFavoriteProducts.map((product) => {
            ...
            const key = `${isSuit ? "suit" : "shirt"}-${product.id}`;
  ...
  "
                ...
                <Link
                  href={isSuit ? `/suit/${product.id}` : `/shirt/${product.id}`}
